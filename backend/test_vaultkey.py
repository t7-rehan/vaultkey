...
class TestVaultKeySecurityWorkflow(unittest.TestCase):
    ...
    def test_full_security_journey(self):
        ...
        print(f"✓ Ciphertext uploaded successfully (File ID: {file_id})")
        ...
        print("✓ Non-PDF file rejection verified")
        ...
        print(f"✓ Share link created (Share ID: {share_id}, Token: {raw_token[:8]}...)")
        ...
        print("✓ Recipient access metadata check verified")
        ...
        print("✓ Encrypted ciphertext retrieved & IV header returned")
        ...
        print("✓ Download limit enforced (429 Limit Reached)")
        ...
        print("✓ Owner revoked share link successfully")
        ...
        print("✓ Recipient access to revoked link denied (403 Revoked)")

        # 10. Audit Activity Log Check
        act_res = client.get(f"/api/activity?file_id={file_id}", headers=headers)
        self.assertEqual(act_res.status_code, 200)
        events = [a["event"] for a in act_res.json()]
        print(f"✓ Security activity events logged: {events}")
