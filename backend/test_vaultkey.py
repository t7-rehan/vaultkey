...
# Set fake env vars before importing app so firebase_admin initialization succeeds
...
class TestVaultKeySecurityWorkflow(unittest.TestCase):
    ...
    def test_full_security_journey(self, mock_del, mock_dl, mock_up):
        ...
        mock_claims = {
            "uid": "alice_firebase_uid_123",
            "email": "alice_test@vaultkey.app"
        }

        with patch("app.security.verify_firebase_token", return_value=mock_claims):
            # 1. Authenticate User via Firebase Token
            token = "mock_firebase_id_token_alice"
            headers = {"Authorization": f"Bearer {token}"}
            print("[OK] Firebase token authentication successful")
